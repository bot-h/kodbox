/*!
 * powered by kodbox;hash:9502a230057097ace1f4 [2020/01/15 18:59:34]
 * 
 */
...
/******/ 		return __webpack_require__.p + "" + ({"3":"vendor"}[chunkId]||chunkId) + ".js?v=" + {"3":"d6f30003"}[chunkId] + ""
/******/ 	}
/******/
...
/******/ 	__webpack_require__.p = "./static/app/dist/";
/******/
...
var bindScroller = function bindScroller() {
	...
	Events.bind('global.file.list.reload.after', scrollerUpdate, window);
};
...
var bindTitle = function bindTitle() {
	...
	$title.poshytip({
		...
		content: function content($tips) {
			$tips.addClass('yellow'); //dark/white/yellow
	...
	$(document).bind('keydown keyup mousedown mouseup click', function () {
...
var bodyEvent = function bodyEvent() {
	var animateOpen = _.get(window.G, 'user.config.animateOpen', '1') == '1' || true;
	if (animateOpen) {
		loadRipple( //点击波纹效果
		'a,button,.ripple-item,.context-menu-item,.kui-btn,.btn', '.disable-ripple,.disabled,.disable,.ztree');
	}
	...
	$('body').delegate('[link-href]', 'click', function (e) {
		var page = $(this).attr('link-href') || '/';
		// url连接;兼容
		if (_.startsWith(page, 'http://') || _.startsWith(page, 'https://')) {
			window.open(page);
			return;
		}
		// 新页面打开: 设置了target=blank; 或鼠标中间
		if (e.which == 2 || $(this).attr('target') == 'blank') {
			var link = API_HOST.replace('index.php?', '');
...
var _globalFeature = __webpack_require__(121);
...
exports.loadPlugin = exports.loadLang = exports.loadOption = exports.loadApi = exports.loadMain = undefined;
...
var initRequire = function initRequire() {
	...
	seajs.use = function () {
		...
		var addVersion = function addVersion(url) {
			...
			if (!version || _.includes(url, '&v=')) return url;
	...
	window._ktime = dateFormate(false, "dhi");
...
var loadPlugin = function loadPlugin() {
	var plugins = API_HOST + 'user/view/plugins&v=' + time();
	return requirePromise(plugins);
};
var loadOption = function loadOption() {
	var options = API_HOST + 'user/view/options&v=' + time();
	return requirePromise("text!" + options).then(function (data) {
		...
		window.G = data.data;
		window.STATIC_PATH = G.kod.staticPath;
		window.VENDER_PATH = G.kod.staticPath + 'app/vender/';
...
var loadLang = function loadLang() {
	var link = API_HOST + 'user/view/lang&v=' + time();
	return requirePromise("text!" + link).then(function (data) {
		if (!data) return;
		var data = JSON.parse(data);
		if (!data || !data.code || !data.data) return;

		window.LNG = _.get(data, 'data.list');
		window.G.lang = _.get(data, 'data.lang');
		window.LNG.find = function (val) {
			var arr = {};
...
/******/ });
//# sourceMappingURL=lib.js.map?v=36fbe08e