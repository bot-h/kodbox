...
return array(
 ...
 ip 간격 : 두 개의 ip는 점선으로 나뉘며 ip는이 간격에서 일치합니다. <br/>",
    ...
    "admin.storage.saveCorsTips"     => "버킷이 CORS로 구성되지 않았거나 구성이 잘못되었습니다. [도움말 사용] 문서에 따라 구성하십시오.",
    ...
    "admin.backup.timeNote"          => "참고 : 지난 7 일 및 매월 1 일의 백업 데이터 만 유지됩니다. <br> 백업 시간 :",
    "admin.backup.optionTime"        => "백업 시간이 오래 걸립니다. 근무 외 시간을 선택해주세요.",
    "admin.backup.optionLocation"    => "백업 전용 스토리지를 생성하십시오",
    "admin.backup.optionTips1"       => "데이터 백업은 파일 백업과 데이터베이스 백업의 두 부분으로 나뉩니다.",
    "admin.backup.optionTips2"       => "먼저 데이터 백업 스토리지 용으로 예약 된 충분한 공간이있는 스토리지를 지정해야합니다.",
    "admin.backup.optionTips3"       => "작업이 시작된 후 파일은 대상 스토리지에 증분 백업되고 데이터베이스는 모든 데이터를 가져 와서 SQL 파일을 생성 한 후 날짜별로 대상 스토리지의 데이터베이스 디렉토리에 백업합니다.",
    "admin.backup.optionTips4"       => "백업 데이터는 지난 7 일과 매월 1 일만 유지합니다.",
 ...
 post_max_size, 다시 시작해야 함)",
    ...
    "log-event-move-more-desc"       => "[1] [2]에서 [3]으로 이동",
    "log-event-moveOut-more-desc"    => "[1]에서 제거됨 [2] [3]",
    "log-event-moveOut"              => "여기에서 제거됨 [1]",
