...
単一行のIPプレフィックス：プレフィックスで始まる場合、IPは一致します。 <br/>
 ip間隔：2つのipは破線で分割され、ipはこの間隔で一致します。 <br/>",
    ...
    "admin.storage.saveCorsTips"     => "バケットがCORSで設定されていないか、設定が間違っています。[ヘルプを使用]ドキュメントに従って設定してください",
    ...
    "admin.backup.position"          => "バックアップ場所",
    ...
    "admin.backup.timeNote"          => "注：過去7日間と毎月1日のバックアップデータのみが保持されます。 <br>バックアップ時間：",
    "admin.backup.optionTime"        => "バックアップに時間がかかります。仕事以外の期間を選択してください",
    "admin.backup.optionLocation"    => "バックアップ専用のストレージを作成してください",
    "admin.backup.optionTips1"       => "データバックアップは、ファイルバックアップとデータベースバックアップの2つの部分に分かれています。",
    "admin.backup.optionTips2"       => "まず、データバックアップストレージ用に予約された十分なスペースがあるストレージを指定する必要があります。",
    "admin.backup.optionTips3"       => "タスクが開始されると、ファイルはターゲットストレージに増分バックアップされ、データベースはすべてのデータを取得し、SQLファイルを生成し、日付によってターゲットストレージのデータベースディレクトリにバックアップします。",
    "admin.backup.optionTips4"       => "バックアップデータは、過去7日間と毎月1日間のみを保持します。",
 ...
 post_max_size、再起動が必要）",
    ...
    "log-event-move-more-desc"       => "[1] [2]から[3]に移動",
    "log-event-moveOut-more-desc"    => "[1]を[2] [3]から削除しました",
