...
return array(
    ...
    "admin.setting.loginIpAllowDesc" => "填写规则如下(默认允许服务器本机ip一级局域网ip):<br/>
单行为ip: 相等则匹配<br/>
单行为ip前缀: ip以前缀为开头则匹配;<br/>
ip区间: 两个ip以中划线进行分割; ip在该区间内则匹配;<br/>",
    ...
    "admin.storage.serverDesc"       => "ftp/s://ip，不加协议则默认为ftp",
    ...
    "admin.storage.saveCorsTips"     => "Bucket没有配置CORS，或配置有误，请按【使用帮助】文档进行配置",
    ...
    "admin.backup.timeNote"          => "说明：仅保留最近7天，及每月1号的备份数据。<br>备份时间：",
    "admin.backup.optionTime"        => "备份需要时间较长，请尽量选择在工作之外的时间段进行",
    "admin.backup.optionLocation"    => "请创建一个专门用于备份的存储",
    "admin.backup.optionTips1"       => "数据备份分文件备份和数据库备份两部分。",
    "admin.backup.optionTips2"       => "首先需指定一个预留有足够空间的存储，专门用于数据的备份存放。",
    "admin.backup.optionTips3"       => "任务启动后，文件增量备份至目标存储，数据库则获取全部数据，生成SQL文件，按日期备份至目标存储的database目录。",
    "admin.backup.optionTips4"       => "备份数据仅保留最近7天，及每月1号的备份。",
...
post_max_size,需要重启)",
    ...
    "log-event-move-more-desc"       => "[1]从[2]移动到[3]",
    "log-event-moveOut-more-desc"    => "[1]从[2]移走了[3]",
